...
class VcxprojGenerator(BaseGenerator):
    ...
    def _GenerateNutBuildCommand(self, project_info: ProjectInfo, configuration: str) -> str:
        ...
        command_lines = [
            ...
            'if !ERRORLEVEL! neq 0 (',
            '    echo Error: Failed to build NutBuildTools (dotnet publish failed)',
            '    exit /b 1',
            ')',
            '',
            'if not exist "%NUTBUILD_BINARY%" (',
            '    echo Error: NutBuildTools binary not found after build',
            '    exit /b 1',
            ')',
            ...
            'set NUTBUILD_EXIT_CODE=!ERRORLEVEL!',
            'if !NUTBUILD_EXIT_CODE! neq 0 (',
            '    echo Warning: NutBuildTools returned exit code !NUTBUILD_EXIT_CODE!',
            '    echo This may be expected for project generation phase',
            ')',
            'echo NutBuildTools execution completed',
            'exit /b 0',
        ]
        return '\n'.join(command_lines)
