...
int main()
{
	...
	for (const auto& className : allClasses)
	{
		...
		const NClassReflection* reflection = NObjectReflection::GetInstance().GetClassReflection(className);
	...
	try
	{
		...
		if (example)
		{
			...
			std::cout << "设置属性: Int=" << example->ExampleIntProperty << ", Float=" << example->ExampleFloatProperty
