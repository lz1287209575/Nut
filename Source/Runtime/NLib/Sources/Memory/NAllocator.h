...
public:
	...
	// 比较操作符
	template <typename U>
	bool operator==(const CAllocator<U>&) const noexcept
	{
...
template <typename TType>
typename CAllocator<T>::pointer CAllocator<T>::allocate(size_type Count)
{
	if (Count == 0)
	{
		return nullptr;
...
template <typename TType>
void CAllocator<T>::deallocate(pointer Ptr, size_type Count) noexcept
{
...
// CString 类型已由 Containers/CString.h 提供，不再使用 type alias
