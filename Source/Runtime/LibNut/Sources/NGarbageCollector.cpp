...
void NGarbageCollector::Initialize(EGCMode Mode, uint32_t CollectionIntervalMs, bool bEnableBackgroundCollection) {
    if (bInitialized) {
        NLogger::Warning("NGarbageCollector already initialized");
        return;
    }
    ...
    NLogger::Info("NGarbageCollector initialized with mode: " + 
...
void NGarbageCollector::Shutdown() {
    ...
    // 清理剩余对象
    {
        ...
        if (!RegisteredObjects.empty()) {
            NLogger::Warning("GC Shutdown: " + std::to_string(RegisteredObjects.size()) + 
                             " objects still registered");
        }
        RegisteredObjects.clear();
        RootObjects.clear();
    }
    
    NLogger::Info("NGarbageCollector shutdown completed. Final collection recovered " + 
                   std::to_string(FinalCollected) + " objects");
}
...
uint32_t NGarbageCollector::Collect(bool bForceFullCollection) {
    ...
    try {
        ...
        NLogger::Info("GC Collection completed: " + std::to_string(TotalCollected) + 
                       " objects collected in " + std::to_string(DurationMs) + "ms");
