...
bool NLib::Initialize(const NLibConfig &Config)
{
    if (bInitialized)
    {
        NLogger::Warn("NLib already initialized");
        return true;
    }
    ...
    {
        // 1. 初始化日志系统
        NLogger::SetLevel(Config.LogLevel);
...
void NLib::Shutdown()
{
    ...
    try
    {
        ...
        {
            NLogger::Warn("Memory leak detected: " + std::to_string(FinalStats.CurrentUsage) +
...
NMemoryManager &NLib::GetMemoryManager()
{
    if (!bInitialized)
    {
...
NGarbageCollector &NLib::GetGarbageCollector()
{
    if (!bInitialized)
    {
        throw std::runtime_error("NLib not initialized. Call NLib::Initialize() first.");
