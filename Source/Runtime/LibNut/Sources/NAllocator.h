...
public:
    ...
    void construct(pointer Ptr, Args&&... Arguments) {
    ...
    // 比较操作符
    template<typename U>
    bool operator==(const NAllocator<U>&) const noexcept {
...
template<typename T>
typename NAllocator<T>::pointer NAllocator<T>::allocate(size_type Count) {
    if (Count == 0) {
        return nullptr;
    }
...
template<typename T>
void NAllocator<T>::deallocate(pointer Ptr, size_type Count) noexcept {
...
using NVector = std::vector<TValue, NAllocator<TValue>>;

template<typename TKey, typename TValue>
using NMap = std::map<TKey, TValue, std::less<TKey>, NAllocator<std::pair<const TKey, TValue>>>;

template<typename TValue>
using NSet = std::set<TValue, std::less<TValue>, NAllocator<TValue>>;

template<typename TValue>
using NList = std::list<TValue, NAllocator<TValue>>;

template<typename TValue>
using NDqueue = std::deque<TValue, NAllocator<TValue>>;
