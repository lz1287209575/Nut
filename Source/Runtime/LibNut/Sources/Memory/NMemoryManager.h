...
class NMemoryManager
{
  public:
    ...
    struct MemoryStats
    {
        ...
        MemoryStats()
            : TotalAllocated(0), TotalFreed(0), CurrentUsage(0), PeakUsage(0), AllocationCount(0), FreeCount(0),
              TcmallocHeapSize(0)
        {
        }
    };
  ...
  public:
     ...
     * @brief 获取内存管理器单例
     */
    static NMemoryManager &GetInstance();
  ...
  private:
    NMemoryManager() = default;
    ~NMemoryManager() = default;

    // 禁用拷贝和赋值
    NMemoryManager(const NMemoryManager &) = delete;
    NMemoryManager &operator=(const NMemoryManager &) = delete;
 ...
 * 可用于标准容器
 */
template <typename T> class TcmallocAllocator
{
  public:
    using value_type = T;
    using pointer = T *;
    ...
    template <typename U> TcmallocAllocator(const TcmallocAllocator<U> &) noexcept
    {
    }

    pointer allocate(size_type n)
    {
        void *ptr = NMemoryManager::GetInstance().Allocate(n * sizeof(T));
        if (!ptr && n > 0)
        {
            throw std::bad_alloc();
        }
    ...
    {
        NMemoryManager::GetInstance().Deallocate(ptr);
    }

    template <typename U> bool operator==(const TcmallocAllocator<U> &) const noexcept
    {
        return true;
    }
...
// === 便利的类型别名 ===
template <typename T> using TcVector = std::vector<T, TcmallocAllocator<T>>;
